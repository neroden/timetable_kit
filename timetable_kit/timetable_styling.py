...
def get_time_column_stylings(
    ...
    """Return a set of CSS attributes or classes to style the header of this column,
...
def style_timetable_for_html(
    ...
    styler map, and style it for output.

    table_classes is a string of extra CSS classes to add to the table; it will always
    have 'tt-table'. table_uuid will have "T_" prefixed (by PANDAS) and be used as an id
    for the table.
    """
...
def make_header_styling_css(header_styling_list, table_uuid: str) -> str:
    """Given a list of strings, which maps from column numbers (the index) to CSS
    attributes, return suitable CSS.

    Assumes PANDAS-standard classes col_heading, col0, col1, etc.  I see no other way to
    do it.  :-(

    The string table_uuid must be supplied. These styling columns apply to the table
    with the id "T_" + table_uuid (conforming to the way PANDAS makes the ids).
    """
    if not table_uuid:
