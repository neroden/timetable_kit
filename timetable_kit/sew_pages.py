...
"""
Wrappers around pdftk-java.  Takes individual one-page PDF timetable files and sews them together.
...
def expand_list_files(files: list[str], *, input_dir) -> list[str]:
    ...
    and expand them into individual JSON filenames.  Copy other filenames through into the
    new list unchanged.  Return the new, longer, list.
    """
    new_list = []
    for x in files:
        if x.endswith(".list"):
...
def assemble_pdf_from_list(filename: str, *, input_dir, output_dir):
    """
    Given the filename for a ".list" file, and assuming the PDF files for the individual pages have been created in output_dir,
    assemble the final multi-page PDF in output_dir.
    """
